...
const [html,runtime,renderer,css,scene]=await Promise.all([
  readFile('index.html','utf8'),readFile('src/runtime/playable-slice.js','utf8'),readFile('src/rendering/canvas-renderer.js','utf8'),readFile('src/runtime/slice.css','utf8'),readFile('src/world/riverford.js','utf8')
]);
...
for(const gameplay of ['moveCharacter(','dog.transform','interact()','startCombat()','applyDamage(','localStorage.setItem','aegisUntil','slashUntil'])if(!(runtime+'\n'+scene).includes(gameplay))throw new Error(`Playable-slice wiring missing: ${gameplay}`);
